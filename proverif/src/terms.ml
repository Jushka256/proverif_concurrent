...
let is_event p = p == Param.event_pred || p == Param.inj_event_pred || p == Param.event2_pred

let is_attacker p = match p.p_info with
  | Attacker _ | AttackerBin _ | AttackerGuess _ -> true
  | _ -> false
...
let term_of_pattern_variable = function
  | PatVar(v) -> Var(v)
  | _ -> internal_error __POS__ "[term_of_pattern_variable] The pattern must be a variable"
...
let mapq_neq f l =
  ...
  in
  List.mapq (List.mapq f_neq) l

let map_constraints f constra =
  {
    neq = mapq_neq f constra.neq;
    is_nat = List.mapq f constra.is_nat;
    is_not_nat = List.mapq f constra.is_not_nat;
    geq = List.mapq (mapq_geq f) constra.geq
  }
...
let rec replace_f_var vl term = match term with
  ...
  | FunApp(f2,l) ->
      let l' =  List.mapq (replace_f_var vl) l in
...
let rec replace_name f t t' = match t' with
  ...
  | FunApp(f',l') ->
      let l'' = List.mapq (replace_name f t) l' in
      if l' == l'' then t' else FunApp(f', l'')
...
let rec get_vars_acc vlist = function
    Var v -> if not (List.memq v (!vlist)) then vlist := v :: (!vlist)
  | FunApp(_,l) ->
      List.iter (get_vars_acc vlist) l

let get_vars_acc_constra vlist c = iter_constraints (get_vars_acc vlist) c

let get_vars_acc_fact vlist = function
    Pred(_,l) -> List.iter (get_vars_acc vlist) l

let rec get_vars_acc_pat accu = function
    PatVar b -> b::accu
  | PatTuple(f,l) -> List.fold_left get_vars_acc_pat accu l
  | PatEqual t -> accu

let rec get_vars_acc_format vlist = function
  | FVar v 
...
let rec reorganize_list l =
  let rec get_first = function
    ...
    | (a ::l)::l' ->
        ...
        (a :: firstl, l :: rest)
    | [] :: _ -> internal_error __POS__ "unexpected case in reorganize_list"
  in
  match l with
    [] :: _ -> []
  | _ ->
...
let get_session_id_from_occurrence = function
  | Var _ -> Parsing_helper.internal_error __POS__ "The term should be an occurrence name, hence a function application"
  | FunApp(n,args) ->
      try
...
let rec choice_in_term n = function
  ...
  | FunApp(f, l) as t ->
      let l' = List.mapq (choice_in_term n) l in
      if l == l' then t else FunApp(f,l')

let choice_in_fact n = function
  | Pred(p,args) as fact ->
      let args' = List.mapq (choice_in_term n) args in
      if args == args' then fact else Pred(p,args')
...
let rec choice_in_pat n pat = match pat with
  ...
  | PatTuple(f,l) ->
      if f.f_cat == Choice then
        raise DiffPattern;
      let l' = List.mapq (choice_in_pat n) l in
...
let make_choice t1 t2 =
  ...
  if (Param.get_ignore_types()) || (ty1 == ty2) then
    ...
    Parsing_helper.internal_error __POS__ "[Terms.make_choice] This should be the same type"
...
let minus_fun =
  Param.memo (fun i ->
    if i <= 0
    then Parsing_helper.internal_error __POS__ "[Terms.minus_fun] Index should be bigger than 0.";
...
let complete_semantics_constructors type_arg type_result =
  ...
  let rec sub_complete var_list var_fail_list fail_list =
    ...
    | _,_,_ -> internal_error __POS__ "The three lists should have the same size"
  in
...
let get_function_name f =
  ...
  | _, Fixed s -> s
  | _ -> Parsing_helper.internal_error __POS__ "get_function_name expects function with fixed name"
  ...
  | Tuple ->
      ...
      then minus_fun 1
        ...
        { f_name = Fixed name;
          ...
          f_record = Param.fresh_record () }
  | _ ->
      internal_error __POS__ "[Terms.projection_fun] This should be a tuple or choice";
      )
...
let rec term_never_fail = function
  ...
  | FunApp(f,args) ->
      begin match f.f_initial_cat with
        ...
        | SpecVar _ | Syntactic _ | General_var | General_mayfail_var -> Parsing_helper.internal_error __POS__ "[Terms.can_never_fail] Unexpected symbol in processes."
      end
...
let copy_name ?orig n arg_type =
  let name =
    match n.f_name with
      Fixed s -> Parsing_helper.internal_error __POS__ "Should only copy names that can be renamed"
...
let generate_destructor_with_side_cond prev_args lht_list rht ext =
  ...
  let rec generalize_prev_args prev_args = match prev_args with
    ...
    | term_list::q ->
        (* Get the variables *)
        let vars = ref [] in
        List.iter (get_vars_acc vars) term_list;
...
let rec fact_list_of_combined_conclusion pl args = match pl with
  | [] ->
      if args <> []
      then Parsing_helper.internal_error __POS__ "[Terms.fact_list_of_combined_conclusion] Conclusion does not match the query.";
      []
  | p :: q_pl ->
...
let fact_list_of_conclusion = function
  | Pred({ p_info = Combined(_,_,pl); _ }, args) ->
...
let rec last_position v last_seen n = function
  | [] ->
      if last_seen = 0
      then Parsing_helper.internal_error __POS__ "[Terms.last_position] The variable should be in the list.";
...
let rec iter_variables f = function
  | Var v -> f v
  | FunApp(_,args) -> List.iter (iter_variables f) args
