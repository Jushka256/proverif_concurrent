...
let check_delayed_names_query = function
  | QueryToTranslate _ ->
      Parsing_helper.internal_error __POS__ "Queries should have been translated before check_delayed_names"
...
let check_delayed_names pi_state =
  ...
  let t_lemmas_list' =
    List.map (function
      | LemmaToTranslate _ -> Parsing_helper.internal_error __POS__ "Lemmas should have been translated before check_delayed_names"
      | Lemma lem_state ->
          let lemmas' =
...
let display_query_header q_type pi_state =
  ...
  if (!Param.tulafale == 1) && (is_corresp initial_query) then
    ...
    begin
      ...
      Parsing_helper.debug (fun () -> Query.Debug.display_t_query q);
      if !Param.html_output then
        begin
          Display.Html.print_string "<LI><span class=\"query\">";
  ...
  Param.current_process_number :=
     ...
     | Some n -> n
     | _ -> Parsing_helper.internal_error __POS__ "Process should have been displayed"

let convert_lemma_to_query lemma =
  if List.for_all (fun t_lem -> t_lem.ql_original_query = None) lemma.lemmas
  then CorrespQuery(List.map (fun t_lem -> t_lem.ql_query) lemma.lemmas,lemma.solve_status)
  else
    ...
    ) lemma.lemmas,lemma.solve_status)
...
let display_axioms pi_state =
  if !current_axioms = [] then
    ...
    List.iter (function
      ...
      | Lemma _ -> ()
      | _ -> Parsing_helper.internal_error __POS__ "[prove_lemmas] All lemmas should have been translated at this point. (2)"
            ) pi_state.pi_lemma

let solve pi_state =
  Parsing_helper.debug (fun () ->
    Parsing_helper.debug_msg "-- Entering solve";
    Parsing_helper.debug_msg (Printf.sprintf "Event considered for second saturation: %s" (String.concat ", " (List.map (fun (f,b) -> Printf.sprintf "%s/%b" (Display.string_of_fsymb f) b) pi_state.pi_events_to_solve_twice)));
  );
  ...
  let pi_state2 = Lemma.organize_lemmas pi_state1 in
  let pi_state3 = Lemma.generate_inductive_lemmas pi_state2 in
...
let rec is_axiom_filtering_needed proc =
  ...
  then
    match proc.construction with
      ...
      | Initial_process1
      | Initial_process2 -> Parsing_helper.internal_error __POS__ "[is_axiom_filtering_needed] The process cannot be a biprocess while being Initial_process 1 or 2"
  else false
...
let get_t_lemmas_from_solved lemma solved_lemmas =
  let solved_lemmas2 =
    List.map (function
        ...
        | _,_ -> Parsing_helper.internal_error __POS__ "[get_t_lemmas_from_solved] Lemmas should only be Correspondence queries."
      ) solved_lemmas
  in
  Lemma { lemma with lemmas = (List.concat solved_lemmas2); solve_status = { lemma.solve_status with induction = false } }

let state_has_restrictions pi_state =
  List.exists (function
    | Lemma { is_axiom = KRestriction } -> true
    | Lemma { is_axiom = KAxiom | KLemma } -> false
    | _-> Parsing_helper.internal_error __POS__ "[state_has_restrictions] All lemmas should have been translated at this point."
          ) pi_state.pi_lemma
...
let prove_lemmas pi_state next_f =
  ...
  let pi_state = Ordering.refresh_ordering_data pi_state in

  let axiom_list,lemma_list =
    List.partition (function
      | Lemma { is_axiom = KAxiom | KRestriction; _} -> true
      | Lemma { is_axiom = KLemma; _} -> false
      | _-> Parsing_helper.internal_error __POS__ "[prove_lemmas] All lemmas should have been translated at this point."
    ) pi_state.pi_lemma
  in
  ...
  let axiom_list' = 
    List.map (function
      | Lemma axioms ->
          let lemmas = 
            List.map (function { ql_query = (q,e); _} as lem ->  
              Ordering.assume_proved q;
              { lem with ql_query = (q,e) }
            ) axioms.lemmas 
          in
          Lemma { axioms with lemmas = lemmas}
      | _ -> Parsing_helper.internal_error __POS__ "[prove_lemmas] All lemmas should have been translated at this point (4)."
    ) axiom_list 
  in
  ...
  let rec prove_all_lemmas proved_lemmas_or_axioms = function
    ...
    | (Lemma lemma)::l_list ->
        ...
        if lemma.solve_status.max_subset
        then
          begin
            ...
            Parsing_helper.debug (fun () -> Lemma.Debug.display_t_lemmas solved_lemmas);
            Ordering.check_lemmas_are_proved solved_lemmas;
    ...
    | _ -> Parsing_helper.internal_error __POS__ "[prove_lemmas] All lemmas should have been translated at this point. (3)"
  in
...
let update_maxsubset pi_state =
  let (p,q) = Param.get_process_query pi_state in
  let q' = match q with
    | CorrespQEnc(qql, s_status) when List.length qql = 1 -> CorrespQEnc(qql,{s_status with max_subset = false})
    | CorrespQuery(ql,s_status) when List.length ql = 1 -> CorrespQuery(ql,{s_status with max_subset = false})
    | _ -> q
  in
...
let prove_queries pi_state =
  match pi_state.pi_process_query with
  | Equivalence _ ->
    if pi_state.pi_requires_liveness
      then Parsing_helper.user_error "With equivalence queries, all restrictions must satisfy the following:\n  All facts in the conclusion of the restriction must occur before one of the fact in the premise (no liveness).\nYou can use the option [conclBeforePremise].";
...
let anal_file s0 =
  ...
  begin
    match in_front_end with
      Default ->
        Parsing_helper.internal_error __POS__ "The Default case should have been removed previously"
    | Horn ->
    ...
    | PiType ->
        ...
        let pi_state5 = Ordering.requires_liveness_and_additional_saturation pi_state4 in
  
        if not !parse_only && has_query pi_state5 then
          begin
            if (!Param.html_output) then
              Display.Html.print_string "<UL>\n";

            Encode_queries.encode_secret_public_vars prove_queries pi_state5;
