...
let update_args_opt lopt =
  ...
  | Some l ->
	...
	| TLink t ->
	    ...
	    b'
	| _ -> Parsing_helper.internal_error __POS__ "unexpected link in Simplify.update_args_opt"
	end) l)
      in
      (new_args_opt, !lets)
...
let rec copy_process copy_info = function
  ...
  | Restr(n,(args,env),p,occ) ->
	...
	| Some(glob_table, glob_table_var_name) ->
           begin
             match n.f_name with
               Fixed _ -> Parsing_helper.internal_error __POS__ "names of restrictions should be renamable"
             | Renamable id ->
...
let prepare_process pi_state =
  begin
    match pi_state.pi_need_vars_in_names with
      Computed (_ :: _) ->
	Parsing_helper.internal_error __POS__ "Simplify.prepare_process should be called before computing pi_need_vars_in_names"
    | _ -> ()
  end;
...
let rec new_occs = function
  ...
  | AnnBarrier _ ->
      Parsing_helper.internal_error __POS__ "Annotated barriers should not appear in reset_occurrence"
...
let rec follow_process = function
  ...
  | Barrier _ | AnnBarrier _ ->
     Parsing_helper.internal_error __POS__ "Barriers should not appear here (8)"
...
let rec remove_from a = function
    [] -> Parsing_helper.internal_error __POS__ "element should be in the list in remove_from"
  | a'::l ->
...
let rec check_disjoint_append l1 l2 =
  ...
  | a::l ->
	...
	Parsing_helper.internal_error __POS__ "collision in check_disjoint_append"
      else
...
let rec check_disjoint_pair_append l1 l2 =
  ...
  | a::l ->
	...
	Parsing_helper.internal_error __POS__ "collision in check_disjoint_pair_append"
      else
...
let one_var_pattern_from_pattern pattern =
  ...
  and sub_one_var_pattern_from_pattern_list cor_term_list pattern_list =
    ...
    | [],[] -> Terms.true_term
    | [],_ | _,[] -> internal_error __POS__ "[one_var_pattern_from_pattern] The two list should have the same size"
    | pat::pat_l, cor::cor_l ->
...
and canonical_process process acc_unify = match process with
  ...
  | Let(pat,term,proc1,proc2,occ) ->
      begin match pat with
		...
		then
		  ...
		  match pat_x with
		    PatVar x ->
		      ...
		      let test = FunApp(Terms.not_caught_fail_fun type_term,[Var x]) in
		      (catch, test)
		  | _ -> Parsing_helper.internal_error __POS__ "[Simplify.canonical_process] patterns returned by one_var_pattern_from_pattern should be variables"
		...
		| _ -> Parsing_helper.internal_error __POS__ "[Simplify.canonical_process] the list of patterns and terms should have the same length"
	      in
  ...
  | LetFilter(_,_,_,_,_) -> internal_error __POS__ "[Simplify.canonical_process] LetFilter should not be in the process"
  | Event ((FunApp(f,args) as term),(_,env),proc,occ) ->
      ...
      then Event(term,(None,env),canonical_process proc acc_unify,occ)
        ...
        generate_tests [] acc_unify args
  | Event(_,_,_,_) -> internal_error __POS__ "[Simplify.canonical_process] Event should be application functions."
  ...
  | Barrier _ | AnnBarrier _ ->
     Parsing_helper.internal_error __POS__ "Barriers should not appear here (9)"
...
let rec norm phase proc = match proc with
  ...
  | Input (c,var,p,occ) ->
      let bind = match var with
        | PatVar (b) -> b
        | _ -> internal_error __POS__ "[Simplify.norm] The pattern should be a variable"
      in
  ...
  | Let(pat,term,proc1,proc2,_) ->
      ...
      let x = match pat with
        | PatVar x -> x
        | _ -> internal_error __POS__ "[Simplify.norm] It should be a variable as pattern"
      in
  ...
  | Get(pat,term,proc1,proc2,occ) ->
      let x = match pat with
        | PatVar x -> x
        | _ -> internal_error __POS__ "[Simplify.norm] It should be a variable as pattern (2)"
      in
  ...
  | Barrier _ | AnnBarrier _ ->
     Parsing_helper.internal_error __POS__ "Barriers should not appear here (10)"
  | _ -> internal_error __POS__ "Filter should not happen"
...
and merge_all_list_R cond procs_R1 seen_R2 procs_R2 next_f =
  ...
  | [],_  -> internal_error __POS__ "[Simplify.merge_all_list_R] Both lists should be of equal length"
  | _ ,[] ->
...
let rec select_leaf_D accu_cond context next_f = function
  ...
  | NormProc(p) ->
      Parsing_helper.internal_error __POS__ "Should not reach the leaves"
  ...
  | Var v ->
      ...
      then
        begin
          ...
          internal_error __POS__ "The previous variable is not declared:\n"
        end;
...
let rec verify_process free_var proc =
  ...
  | Barrier _ | AnnBarrier _ ->
     Parsing_helper.internal_error __POS__ "Barriers should not appear here (11)"
...
let simplify_state state next_f =
  let process_desc, query =
    ...
    | _ ->
	Parsing_helper.internal_error __POS__ "Simplify.simplify_state should be called only for equivalence or choice queries"
  in
