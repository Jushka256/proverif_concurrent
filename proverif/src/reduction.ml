...
let noninterftest_to_string = function
  ...
  | NIFailTest _ -> Parsing_helper.internal_error __POS__ "Unexpected FailTest for noninterf"
...
let get_occurrence_name_for_precise occ name_params =
  ...
  match n.f_cat with
    ...
    | _ -> Parsing_helper.internal_error __POS__ "[Reduction.get_occurrence_name_for_precise] Unexpected case."
          ...
          auto_cleanup_red (fun () ->
            ...
            with Unify ->
              ...
              let false_required_for_else_branch =
                List.map (fun cl -> 
                  let (hyp, _, _, _) = Clause.Ord.reduction_of cl in
...
let test_success cur_state' =
  try
    match cur_state'.goal with
      CorrespGoal(number_original_goals,l,ids) ->
        let new_goal =
          CorrespGoal (number_original_goals,List.map (fun goal ->
            match goal with
          ...
          *)
          ...
          | Apply (f,_) when f.f_cat != Tuple ->
            begin
              let (p,c) =
                ...
                | _ -> Parsing_helper.internal_error __POS__ "unexpected Apply clause"
              in
              let h = List.map (function
              { thefact = Pred(_,[t]) } -> (new_var ~orig:false "~X" (get_term_type t), rev_name_subst t)
                |	_ -> Parsing_helper.internal_error __POS__ "unexpected Apply clause") sons
              in
          ...
          | Rn _ ->
            begin
              ...
              | _ ->
                Parsing_helper.internal_error __POS__ "Rule Rn should conclude p(name)"
            end
          | _ -> state1
        end
      | h::t ->
        ...
        try
          auto_cleanup_red (fun () ->
            ...
            let new_facts = match new_occs with
              ...
              | (InputTag _) :: _ -> [fact']
              | _ -> Parsing_helper.internal_error __POS__ "[Reduction.do_async_res_io] First element of new_occs should be an input tag."
            in
              (* Do RIO *)
...
exception Unexpected_derivation
...
let rec analyze_weak_secr_tree_rec accu weaksecrets tree =
    ...
    FRule(_, lbl, _, hyp,_) ->
      begin
		...
		if ((!Param.max_hyp) >= 0) || ((!Param.max_depth) >= 0) then
		  raise Unexpected_derivation;
		Parsing_helper.internal_error __POS__ "Unexpected WeakSecr clause in the derivation for weaksecret"
          end
        | PhaseChange ->
          begin
            match tree.thefact with
        ...
        | _ -> Parsing_helper.internal_error __POS__ "Unexpected clause in derivation for weaksecret"
      end
  ...
  | FHAny ->
	...
	| Pred({ p_info = AttackerGuess _ }, [t1;t2]) ->
	    rev_name_subst t2
	| _ -> Parsing_helper.internal_error __POS__ "Unexpected WeakSecr clause in the derivation for weaksecret"
      end
  | FEmpty | FRemovedBySimplification ->
      if ((!Param.max_hyp) >= 0) || ((!Param.max_depth) >= 0) then
	raise Unexpected_derivation;
      Parsing_helper.internal_error __POS__ "Unexpected derivation for weaksecret"
  ...
  match tree.desc with
    FRule(_, lbl, _, hyp,_) ->
      begin
	    ...
	    if ((!Param.max_hyp) >= 0) || ((!Param.max_depth) >= 0) then
	      raise Unexpected_derivation;
	    Parsing_helper.internal_error __POS__ "Unexpected clause concluding the derivation for weaksecret"
      end
  | _ -> 
      if ((!Param.max_hyp) >= 0) || ((!Param.max_depth) >= 0) then
	raise Unexpected_derivation;
      Parsing_helper.internal_error __POS__ "Unexpected derivation for weaksecret (head)"
      ...
      begin
        ...
        | ProcessRule(hyp_tags, name_params), hyp ->
            ...
            (* Pattern-matching test in an input *)
                  let mess_term =
                    ...
                    | _ -> Parsing_helper.internal_error __POS__ "Unexpected derivation for noninterf: input with fact other than att/mess"
                  in
        ...
        | TestApply(f, p), _(*testunif fact*)::hyp ->
          ApplyTest(f, List.map (function
            ...
            | _ -> Parsing_helper.internal_error __POS__ "Unexpected derivation for noninterf")
                      hyp)
        ...
        | _ -> Parsing_helper.internal_error __POS__ "Unexpected clause concluding the derivation for noninterf"
      end
  | _ -> Parsing_helper.internal_error __POS__ "Unexpected derivation for noninterf"
...
let build_goal_index_and_event_table final_state =
  ...
  let rec explore_state state =
    ...
    let corresp_goal = match state.goal with
      | CorrespGoal (number_original_goals,goal_list,_) -> Utils.List.crop number_original_goals goal_list
      | _ -> Parsing_helper.internal_error __POS__ "[Reduction.build_goal_index_and_event_table] Unexpected goal."
    in
...
let rec link_occurrence occ step = match occ with
  ...
  | Var ({ link = SLink _; _}) -> Parsing_helper.internal_error __POS__ "[Reduction.link_occurrence] the occurrence term is already linked to a step."
...
let update_max_step_with_ord_fun max_step eord_fun prem_steps = match eord_fun with 
  | QOSpecific eord_list ->
      let rec explore i max_step ord_fun prem_steps = match ord_fun,prem_steps with
        | [],[] -> max_step
        | _, [] -> Parsing_helper.internal_error __POS__ "[Reduction.update_max_step_with_ord_fun] The premise steps do not match the ordering functions"
        | [], _ -> max_step
        | (j,_)::_, _::q_step when i < j -> explore (i+1) max_step ord_fun q_step
        | (j,_)::_, _ when i > j -> Parsing_helper.internal_error __POS__ "[Reduction.update_max_step_with_ord_fun] Bad index"
        | (_,(SemStd,Leq))::q_ord,step::q_step -> explore (i+1) (min max_step step) q_ord q_step
        | (_,(SemStd,Less))::q_ord,step::q_step -> explore (i+1) (min max_step (step-1)) q_ord q_step
        | _ -> Parsing_helper.internal_error __POS__ "[Reduction.update_max_step_with_ord_fun] Unexpected order"
      in
      explore 1 max_step eord_list prem_steps
  | _ -> max_step
...
let rec extract_conclusion_query restwork nested_op = function
  ...
  | QEvent (QGr _) -> Parsing_helper.internal_error __POS__ "[Reduction.extract_conclusion_query] QGr should not occur in query during attack reconstruction."
...
let match_one_inj_event restwork ev0 step0 k inj_steps b event_table =
  ...
  then
    ...
    let rec explore seen_event_table = function
      | [] -> Parsing_helper.internal_error __POS__ "[Reduction.find_one_inj_event_table] The step should be in the event table."
...
let match_query_event restwork max_step prem_steps inj_steps b event_table = function
  ...
  | _ -> Parsing_helper.internal_error __POS__ "[Reduction.match_query_event] We should expect a query event"
...
let check_occurrence_neq occ_neq_l =
  List.iter (fun (i,j) -> match get_step_from_occurrence_term i, get_step_from_occurrence_term j with
    | Some step, Some step' ->
        if step = step'
        then raise Unify
    | _ -> Parsing_helper.internal_error __POS__ "[Reduction.check_occurrence_neq] All occurrence variables should have been linked by now."
  ) occ_neq_l
  ...
  extract_conclusion_query (fun (evlist, facts, constra, occ_neq) ->
    match_query_event_list (fun event_table' ->
      ...
      TermsEq.close_constraints_eq_synt (fun constra' ->
	    ...
	    in
            let rule = (facts, bad_fact, Rule(-1, LblNone, facts, bad_fact, constra), constra) in
            let cl = Clause.Ord.of_reduction rule in
              (* check the facts and constraints by resolution *)
            if (Terms.auto_cleanup (fun () -> Rules.solving_request_rule [] cl)) != [] then
...
let all_events_to_check concl_q_accu concl_q occ_step goal_step evl event_table constra_prem =
  ...
  let ev_term_occ_l =
    List.map (function
      | QSEvent(Some _,_,Some occ,ev) -> ev,occ
      | _ -> Parsing_helper.internal_error __POS__ "[Reduction.all_events] All events should be injective with an occurrence variable"
    ) evl
  in
...
let is_non_inj_neq = function
    QSEvent(inj,ord_fun,occ,_) -> inj = None && occ = None
  | QNeq _ -> Parsing_helper.internal_error __POS__ "inequalities should not occur in left-hand side of ==> in queries"
  | _ -> true
...
let add_fresh_occurrence_event = function
  | QSEvent(inj,ord_fun,None,ev) ->
      let occ = Terms.new_var_def_term Param.occurrence_type in
      QSEvent(inj,ord_fun,Some occ,ev)
  | qev -> qev
...
let rec add_fresh_occurrence_concl_query = function
  ...
  | NestedQuery _ -> Parsing_helper.internal_error __POS__ "[Reduction.add_fresh_occurrence_event] Nested queries should have a unique event as premise."
...
let rec are_non_inj_event_and_facts_equal ev_query goall1 goall2 step_goall1 step_goall2 =
  ...
  | QSEvent(Some _,_,_,_)::_, _, _, _, _ -> true
...
let rec check_query_falsified_rec restwork max_step b event_table constra_prem concl_q gen_occ_step gen_goal_step evl goall goal_stepl =
  ...
  | ev::rest_evl, (Fact(goal,_,_) | EventGoal(goal,_))::rest_goall, step::rest_goal_stepl ->
      let (l,l',new_gen_occ_step) =
        match ev, goal with
          QFact(p,_,l), Pred(p',l') when p == p' -> l,l', gen_occ_step
        | QSEvent(None,_,Some occ,t), Pred(pr,[t']) when pr == Param.event_pred -> [t],[t'],(occ,step)::gen_occ_step
        | QSEvent(None,_,Some occ,t), Pred(pr,[t';_]) when pr == Param.inj_event_pred -> [t],[t'],(occ,step)::gen_occ_step
        | _ ->
  ...
  if has_name_q q then
    ...
    *)
    ...
    match final_state.goal with
      CorrespGoal(number_original_goals,goall,id_goals) ->
        begin
          ...
          then
            ...
            let (goall1, goall2) = List.split_nth size_evl goall' in
            let (id_goals_step1,id_goals_step2) = List.split_nth size_evl id_goals_step in
    ...
    | _ ->
        Parsing_helper.internal_error __POS__ "The goal of the trace does not match the query (4)"
  ...
  try
    Reduction_helper.instantiate_natural_predicates (fun () ->
      ...
      let goal =
		...
		if ((!Param.max_hyp) >= 0) || ((!Param.max_depth) >= 0) then
		  raise Unexpected_derivation;		 
                Parsing_helper.internal_error __POS__ "weak secret clause should appear"
             | Some (w1,w2) ->
        ...
        | CorrespQuery _ | CorrespQEnc _ ->
            let (nb_pure_goal,goal_list,id_goals) = get_corresp_goals tree in
        ...
        | _ ->
           Parsing_helper.internal_error __POS__ "Unexpected query in reduction.ml"
      in
      let init_state =
        { goal = goal;
