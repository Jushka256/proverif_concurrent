...
let unify_up_to_x ?(id_thread=0) next_stage t1 t2 =
  ...
  try
    ...
    let keep = List.for_all (fun (f,v) ->
      ...
      | _ -> Parsing_helper.internal_error __POS__ "unexpected link in unify_up_to_x") vlsecr
    in
    if not keep then
      cleanup()
    else
...
let rec swap t1done t2done t1remain t2remain =
  ...
  | _ -> Parsing_helper.internal_error __POS__ "Lists of different lengths in swap"
...
module Make 
  ...
  NonInterfSig with type clause = C.clause =
struct
  ...
  let is_standard_clause = function
    | { hypotheses = [h1;h2;h3]; conclusion = Pred(p4, []); constraints = c } ->
        begin match H.get_fact h1, H.get_fact h2, H.get_fact h3 with
          | Pred(p1, [Var v1]), Pred(p2, [Var v2]), Pred({p_info = TestUnifP _}, [Var v1'; Var v2']) when 
  ...
  let swap_with_copy next_stage hypbefore hypafter concl hist constra l1 l2 =
    ...
    let hypbefore', hypafter', concl', constra' = 
      Terms.auto_cleanup_noexception (fun () ->
        List.map H.copy3 hypbefore,
        List.map H.copy3 hypafter,
        Terms.copy_fact3 concl,
        Terms.copy_constra3 constra
      )
    in
  ...
  ****)
  ...
  let dec_out_rule next_stage hypbefore hypafter concl hist constra t1simp t2simp =
    match (t1simp,t2simp) with
      [],[] -> ()
    | [t1s],[t2s] ->
        let hypbefore' = H.create_no_ordering_allowed (Pred(Param.get_pred (TestUnifP(Terms.get_term_type t1s)), [t1s; t2s])) :: hypbefore in
        next_stage hypbefore' hypafter concl hist constra
    | _ ->
        let tuple_fun = get_tuple_fun (List.map Terms.get_term_type t1simp) in
        let hypbefore' = H.create_no_ordering_allowed (Pred(Param.get_pred (TestUnifP(Param.bitstring_type)),
                              [ FunApp(tuple_fun, t1simp);
                                FunApp(tuple_fun, t2simp)])) :: hypbefore
        in
        next_stage hypbefore' hypafter concl hist constra

  let check_any_x hyp = match H.get_fact hyp with
  ...
  let secr_in_sets next_stage hypbefore hypafter concl hist constra t1 t2 =
    ...
    let rec unify_secrets todo = function
        [] ->
          ...
          let hypbefore', hypafter', concl', constra' = 
            Terms.auto_cleanup_noexception (fun () ->
              List.map H.copy2 hypbefore,
              List.map H.copy2 hypafter,
              Terms.copy_fact2 concl,
              Terms.copy_constra2 constra
            )
          in
          (* Auto-cleanup is needed here because
	     [Terms.current_bound_vars] may be non-empty due to unifications
	     performed here, while [next_stage] typically expects an
	     empty [Terms.current_bound_vars].
	     [next_stage] cannot raise exceptions. *)
          Terms.auto_cleanup_noexception (fun () ->
            next_stage hypbefore' hypafter' concl' hist constra'
          ) 
      | ((f,v)::l) ->
    ...
    try
      ...
      cleanup()
    with Unify ->
      Parsing_helper.internal_error __POS__ "Here, the terms should be unifiable"
  ...
  ****)
  ...
  let is_att_x x hyp = match H.get_fact hyp with
  ...
  let inst_elim next_stage repeat_next_stage hypbefore hypafter concl hist constra l1 l2 =
    ...
    let contains_att_x x = (List.exists (is_att_x x) hypbefore) || (List.exists (is_att_x x) hypafter) in
    let is_unselectable_or_removed hyp =
      let f = H.get_fact hyp in
    ...
    let redo_optim () =
        let hypbefore',hypafter',concl', constra', l1',l2' = 
          
            List.map H.copy2 hypbefore,
            List.map H.copy2 hypafter,
            Terms.copy_fact2 concl,
            Terms.copy_constra2 constra,
            List.map Terms.copy_term2 l1,
            List.map Terms.copy_term2 l2
          
        in
        Terms.cleanup();
        dec_out_rule (fun hypbefore hypafter concl hist constra ->
          repeat_next_stage 
            { hypotheses = List.rev_append hypbefore hypafter;
              conclusion = concl;
              history = hist;
              constraints = constra
            }
  ...
  ****)
      ...
      let rec simplify_testunif hypbefore hypafter concl hist constra =
        match hypafter with
          [] -> 
            next_stage 
              { 
                hypotheses = List.rev hypbefore;
                conclusion = concl;
                history = hist;
                constraints = constra
              }
        |	(a::l) ->
            match H.get_fact a with
  ...
  let selfun cl =
    if not ((!non_interference) && (Terms.is_bad cl.conclusion) && (cl.hypotheses != [])) then -1 else
    if is_standard_clause cl then 0 else
    begin
