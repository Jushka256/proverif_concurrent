...
let rec check_red = function
  ...
  | (env, Clause(i,c))::l ->
      ...
      let _,ext = c in 
      Transl_helper.check_membership ext memberOptim hyp constra concl;
  ...
  | (env, Equiv(i,c,select))::l ->
      ...
      let concl = check_simple_fact env c in
      let _,ext = c in
      Transl_helper.forbid_equiv_membership ext memberOptim hyp concl;
      let requiv =
	incr rule_counter;
...
let gen_data_clauses () =
  let output_rule_hist h =
    ...
    | _ -> Parsing_helper.internal_error __POS__ "unexpected result in output_rule_hist"
  in
  ...
  Hashtbl.iter (fun _ p ->
	...
	then
	  ...
	  Parsing_helper.internal_error __POS__ "In the typed front-end, only polymorphic predicates can be declated data"
      end
	       ) pred_env
...
let rec check_all = function
  ...
  | [TReduc r] ->
      check_red r;
      gen_data_clauses()
  | _ -> internal_error __POS__ "The first reduc part is not the last element of the file"
...
let interpret_setting (p,ext) v =
  ...
  | "ignoreTypes", _ ->
      Parsing_helper.input_error ("Unexpected value for parameter " ^ p ^ "=" ^ (Param.string_of_pval v)) ext
...
let parse_file s =
  ...
  let set_memberOptim = Transl_helper.set_membership_predicates !rules !memberOptim in
  ...
  let state =
    { h_clauses = Given !rules;
      ...
      h_pred_prove = [];
      h_event_in_queries = [];
      h_predicates_for_second_saturation = [];
      h_events_for_second_saturation = [];
      h_ordered_saturation = false
    }
  in
