...
type ordering_data = 
  { 
    ord_target : query_ordering_relation; (* The ordering function objective *)
    ord_proved : query_ordering_relation option ref; (* The ordering function actually proved (stricter than ord_target) *)
    temp_var : term_e option (* Temporal variable *)
  }

type event =
    QSEvent of
      int option (* None when not injective. Some k indicates injective with k as index. *) *
      ordering_data (* Ordering information *) *
      term option (* Occurrence term *) *
      term (* The event *)
  | QSEvent2 of 
      ordering_data (* Ordering information *) *
      term * term (* Event for biprocesses *)
  | QFact of
      predicate *
      ordering_data (* Ordering information *) *
  ...
  | QGr of term_e * term_e (* Only for time variables *)
  | QIsNat of term
  | QMax of term * term list
  | QMaxq of term * term list
...
and t_one_lemma =
  {
    ...
    ql_index_query_for_induction : int option 
      (** When a list of queries [[q0;...;qn]] must be proved by induction, the generated lemmas record to which queries
          they correspond. For example if [Some i] is given, then it indicates that it refers to the query [qi]. *)
  }
...
and t_lemmas_state =
  {
    ...
    solve_status : t_solve_corresp_status
  }
...
and t_solve_corresp_status =
  {
    max_subset : bool; (** When [false], all queries in the group must be proven to be true. *)
    sat_app : bool; (** Application condition during the saturation *)
    verif_app : bool; (** Application condition during the verification *)
    induction : bool; (** When [true], the group of queries must be proved by induction. *)
    concl_before_premise : bool (** When [true], all facts in the conclusion of a query from the group
      must occur before at least one of the facts in the premise of that query. *)
  }
...
and t_pi_state =
    { pi_process_query : t_process_query;
      ...
      pi_memberOptim : (predicate * funsymb option) list;
        (** Predicates that have been given with option memberOptim *)
      ...
      pi_originally_lemma : bool;
        (* true indicates that the query was originally a lemma. *)
      
      pi_requires_liveness : bool;
        (** When [false], then all queries, lemmas, restrictions, and axioms can be considered as if the option 
            the option [conclBeforePremise] was given to each of them. In particular, we can prove the lemmas
            and queries without ordering.
            Liveness is required when a restriction has been declared with an event in the conclusion not ordered
            with at least one of the premises of the restriction. *)

      (** We may run a second round of saturation before entering the verification stage.
          Note that we may require additional saturation even if liveness is not required. In particular, 
          when a restriction has been declared with an event in the
          conclusion or if some predicate or event has been declared with option [solveTwice] *)
              
      pi_events_to_solve_twice : (funsymb * bool) list;
        (** List of events that are considered for second
          saturation. An element [(ev,true)] indicates that [ev] has
          been declared with [solveTwice]. An element [(ev,false)]
          indicates that [ev] has been declared with [solveOnce] and
          cannot be solved in the second saturation even if it appears
          in the conclusion of a restriction. *)

      pi_predicates_to_solve_twice : (predicate * bool) list
        (** Similar than [pi_events_to_solve_twice] but for predicates. *)
    }
...
type 'a goal_t =
  | CorrespGoal of
      int (* number of original goals *) *
      corresp_goal_t list (* All the goals to be reached *) * 
      int list (* The goals corresponding to the initial query *)
