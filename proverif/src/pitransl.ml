...
let verify_explored_occurrence cur_state process f_next =
  ...
  then f_next ()
    ...
    match process with
      ...
      | Barrier _ | AnnBarrier _ -> internal_error __POS__ "[verify_explored_occurrence] Barriers should not appear here."
      ...
      Terms.auto_cleanup (fun _ ->
        ...
        begin
          try
            ...
            TermsEq.simplify_constraints_continuation get_vars_op (fun constra3 ->
        ...
        if !Param.key_compromise = 2 then
          begin
            ...
            try
              let get_vars_op = Some (fun () -> Terms.get_vars_fact_list (concl::hyp)) in
              TermsEq.simplify_constraints_continuation get_vars_op (fun constra4 ->
...
let check_feasible f_next check cur_state =
  ...
  then
    ...
    try
      let last_constra = TermsEq.simplify_constraints_simple None cur_state.last_step_constra in
    ...
    let check_constraints () =
      ...
      then
        ...
        f_next
          { cur_state with
            constra = TermsEq.simplify_constraints_simple None (Terms.wedge_constraints cur_state.last_step_constra cur_state.constra);
            last_step_unif_left = [];
            last_step_unif_right = [];
...
let end_destructor_group_no_test_unif next_f cur_state =
  ...
  then next_f cur_state
    ...
    cleanup_bound_vars (fun () ->
      ...
      let constra = TermsEq.simplify_constraints_simple None (Terms.wedge_constraints cur_state.last_step_constra cur_state.constra) in
                  ...
                  let (eq_left,eq_right) =
                    Terms.auto_cleanup (fun _ ->
                      List.fold_right (fun (v,link) (acc_l,acc_r) -> match link with
                        ...
                        | _ -> Parsing_helper.internal_error __POS__ "[end_destructor_group] Variables should be linked."
                      ) vars_links ([],[])
                    )
                  in
...
let rec transl_rewrite_rules next_f cur_state check transl_args args pos rules =
  ...
  then transl_rewrite_rules_execute next_f cur_state transl_args args pos rules
    ...
    match rules with
      | [] -> ()
      | (ToCheck(i,_),_)::_
      | (ToExecute i, _):: _ when pos > i -> Parsing_helper.internal_error __POS__ "[transl_rewrite_rules] Should have been handled before";
      | (ToCheck(i,_),_)::_
	  ...
	  List.iter (function
	    ...
	    | _ -> Parsing_helper.internal_error __POS__ "[transl_rewrite_rules_one_side] should only contain ToCheck"
		  ) check_rules;
      ...
      match f.f_cat with
        Name n ->
          begin
            ...
            | _ -> internal_error __POS__ "unexpected prev_inputs in transl_term"
          end
        ...
        if fst n.f_type == Param.tmp_type then
          ...
          begin
            ...
            if List.length (fst n.f_type) != List.length nptype then
              internal_error __POS__ ("Name " ^ n_string ^ " has bad arity")
          end
        else
          begin
            if not (Terms.eq_lists (fst n.f_type) nptype) then
              internal_error __POS__ ("Name " ^ n_string ^ " has bad type")
          end;
...
let rec transl_process cur_state process =
  verify_explored_occurrence cur_state process (fun () -> match process with
	       ...
	       assert (cur_state.record_fun_opt = None);
             ...
             begin
               (* When we ignore types, the types of the arguments may vary,
                  only the number of arguments is preserved. *)
               if List.length (fst n.f_type) != List.length nptype then
                 internal_error __POS__ ("Name " ^ (Display.string_of_fsymb n) ^ " has bad arity")
             end
           else
             begin
               if not (Terms.eq_lists (fst n.f_type) nptype) then
        ...
        let get_occurrence_name cur_state =
          ...
          match n.f_cat with
            | Name r ->
                ...
                FunApp(n,np)
            | _ -> internal_error __POS__ "[transl_process] Unexpected function category in the translation of events."
        in
    ...
    | Barrier _ | AnnBarrier _ ->
        internal_error __POS__ "Barriers should not appear here (3)"
  )
...
let rec comp_transl_process = function
 ...
 | Restr(n,_,p,_) ->
     begin
       match n.f_cat with
         Name { prev_inputs_meaning = l } ->
           let rec build_params ml tl =
             ...
             | _ -> Parsing_helper.internal_error __POS__ "bad length in comp_transl_process"
           in
       ...
       | _ -> internal_error __POS__ "name expected in comp_transl_process"
     end;
     comp_transl_process p
 | Test(t1,p,q,_) ->
     comp_transl_process p;
 ...
 | Barrier _ | AnnBarrier _ ->
     internal_error __POS__ "Barriers should not appear here (4)"
...
let rec apply_mapping_process m = function
  ...
  | Barrier _ | AnnBarrier _ -> Parsing_helper.internal_error __POS__ "[apply_mapping_process] Barriers should not appear here"

let set_free_names_prev_inputs f_next pi_state =
  List.iter (fun ch ->
    match ch.f_cat with
      Name r -> r.prev_inputs <- Some (FunApp(ch, []))
    | _ -> internal_error __POS__ "should be a name 1")
  ...
  List.iter (fun ch ->
    ...
    | _ -> internal_error __POS__ "should be a name 2")
    pi_state.pi_freenames
...
let add_pred_prove_and_events_event (accu_pred,accu_ev) = function
  | QSEvent(_,_,_,FunApp(ev,_)) | QSEvent2(_,FunApp(ev,_),_) -> (accu_pred,add ev accu_ev)
  | QSEvent _ | QSEvent2 _ -> internal_error __POS__ "[add_pred_prove_and_events_for_lemmas_event] Events predicate should have an event symbol."
  | QNeq _ | QEq _ | QGeq _ | QGr _ | QIsNat _ -> (accu_pred,accu_ev)
  | QMax _ | QMaxq _ -> internal_error __POS__ "[add_pred_prove_and_events_event] Max query should not occur here."
  | QFact(p,_,_) ->
      match p.p_info with
...
let add_pred_prove_and_events_query accu (q0, ext) =
  ...
  | QSecret _ ->
      Parsing_helper.internal_error __POS__ "QSecret should have been encoded"
...
let transl pi_state =
  ...
  Transl_helper.remove_subsumed_attacker_clauses_before_display red_rules nrule;

  let memberOptim = Transl_helper.set_membership_predicates !red_rules pi_state.pi_memberOptim in
  ...
  let solver_kind =
    ...
    | CorrespQuery _ | CorrespQEnc _ ->
        Solve_Standard
    | _ -> Parsing_helper.internal_error __POS__ "unsupported query in pitransl"
  in
  ...
  let horn_state =
    { h_clauses = ToGenerate (List.rev (!red_rules), generate_process_clauses);
      ...
      (* Second derivations *)
      h_predicates_for_second_saturation = List.filter_map (fun (p,b) -> if b then Some p else None) pi_state.pi_predicates_to_solve_twice;
      h_events_for_second_saturation = List.filter_map (fun (ev,b) -> if b then Some ev else None) pi_state.pi_events_to_solve_twice;

      h_ordered_saturation = pi_state.pi_requires_liveness || !Param.ordered_saturation
    }
  in
  reset();
  horn_state, pi_state'
