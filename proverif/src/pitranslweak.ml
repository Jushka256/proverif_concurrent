        ...
        try
          ...
          begin
            try
              ...
              TermsEq.simplify_constraints_continuation get_vars_op (fun constra1 ->
...
let verify_explored_occurrence cur_state process f_next =
  ...
  then f_next ()
    ...
    match process with
      ...
      | Barrier _ | AnnBarrier _ -> internal_error __POS__ "[Pitranslweak.verify_explored_occurrence] Barriers should not appear here."
...
let rec get_term_side get_left = function
  | Var v as t -> 
      ...
      | TLink t -> get_term_side get_left t
      | _ -> t
      end 
...
let rec transl_rewrite_rules_one_side next_f cur_state do_left check_c transl_t to_transl pos rules =
  ...
  then transl_rewrite_rules_one_side_execute next_f cur_state do_left check_c transl_t to_transl pos rules
    ...
    match rules with
      | [] -> ()
      | (ToCheck(i,_),_)::_
      | (ToExecute i, _):: _ when pos > i -> Parsing_helper.internal_error __POS__ "[Pitranslweak.transl_rewrite_rules] Should have been handled before";
      | (ToCheck(i,_),_)::_
      ...
      | _ ->
          ...
          List.iter (function
            | ToCheck(i,j),(l,r,c) when i = j ->
                Terms.auto_cleanup (fun () ->
                  try
                    ...
                    let cur_state' =
                      if check_c
                      then { cur_state with constra = TermsEq.simplify_constraints_simple None (Terms.wedge_constraints c cur_state.constra) }
                      else cur_state
            ...
            | _ -> Parsing_helper.internal_error __POS__ "[Pitranslweak.transl_rewrite_rules_one_side] should only contain ToCheck"
          ) check;
      ...
      match f.f_cat with
        Name n ->
          begin
            ...
            | _ -> internal_error __POS__ "unexpected prev_inputs in transl_term_one_side"
          end
...
let get_var_link cur_state v =
  ...
  | TLink t ->
	...
	| _ -> internal_error __POS__ "variable should be linked to choice"
      else
	t
  | _ -> internal_error __POS__ "unexpected link in translate_term (4)"
...
let rec transl_pat next_f cur_state under_choice pattern =
  ...
  | PatVar b ->
	...
	if cur_state.cur_phase < !min_choice_phase || under_choice then
      ...
      in
      Terms.link_unsafe b (TLink (FunApp(Param.choice_fun b.btype, x_pair)));
...
let generate_pattern_with_uni_var binders_list term_pat_left term_pat_right =
  ...
  let new_var_pat_l = List.map (fun v ->
    match Terms.follow_link (fun b -> Var b) (Var v) with
      |Var v' -> v'
      |_ -> internal_error __POS__ "unexpected term in translate_process (3)") var_pat_l

  and new_var_pat_r = List.map (fun v ->
    match Terms.follow_link (fun b -> Var b) (Var v) with
      |Var v' -> v'
      |_ -> internal_error __POS__ "unexpected term in translate_process (4)") var_pat_r in
		    ...
		    assert (cur_state.record_fun_opt = None);
                  ...
                  begin
                    ...
                    if List.length (fst name.f_type) != List.length name_prev_type then
                      internal_error __POS__ ("Name " ^ (Display.string_of_fsymb name) ^ " has bad arity")
                  end
                else
                  begin
                    if not (Terms.eq_lists (fst name.f_type) name_prev_type) then
                      internal_error __POS__ ("Name " ^ (Display.string_of_fsymb name) ^ " has bad type")
                  end;
    ...
    | Event(FunApp(f,_) as t,_,p,occ) ->
        ...
        transl_term (fun cur_state1 term_left term_right ->
          ...
          begin match fstatus.begin_status with
            ...
            | NoOcc ->
                transl_both_side_succeed (fun cur_state2 ->
                  (* TODO: Verify that in equivalence queries, only events in restrictions can have end_status <> No *)
                  begin match fstatus.end_status with
                    | No -> ()
                    | NoOcc ->
                        output_rule { cur_state2 with
...
let add_events_event accu = function
  | QSEvent(_,_,_,FunApp(ev,_)) | QSEvent2(_,FunApp(ev,_),_) -> add ev accu
  | QSEvent _ | QSEvent2 _ -> internal_error __POS__ "[Pitranslweak.add_events_event] Events predicate should have an event symbol."
  | QNeq _ | QEq _ | QGeq _ | QGr _ | QIsNat _ | QFact _ -> accu
  | QMax _ | QMaxq _ -> internal_error __POS__ "[Pitranslweak.add_events_event] QMax and QMaxq should not occur here."
...
let add_events_query accu = function
  ...
  | _ -> Parsing_helper.internal_error __POS__ "[Pitranslweak.add_events_query] Unexpected query for lemmas."
...
let transl pi_state =
  ...
  let p =
    ...
    | SingleProcessSingleQuery(p, (CorrespQEnc _ | CorrespQuery _)) ->
        ...
        p.proc
    | _ -> Parsing_helper.internal_error __POS__ "query not supported by pitranslweak"
  in
  ...
  min_choice_phase :=
     begin
       ...
       | Unset -> Parsing_helper.internal_error __POS__ "Pitranslweak.transl: pi_min_choice_phase should be set"
     end;
  ...
  let horn_state =
    { h_clauses = ToGenerate (List.rev (!red_rules),generate_process_clauses);
      ...
      h_solver_kind = if !for_equivalence then Solve_Equivalence else Solve_CorrespBipro;
      h_lemmas = [];
      h_pred_prove = []; 
