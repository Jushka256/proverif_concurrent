...
let rec eq_proc ren1 ren2 p1 p2 =
  ...
  | Barrier _, _ | _, Barrier _ ->
      Parsing_helper.internal_error __POS__ "Unannotated barriers should not occur in eq_proc"
...
let rec has_destr = function
  ...
  | FunApp(f,l) ->
	...
	Red _ | Failure -> true
      |	Syntactic _ ->
	  Parsing_helper.internal_error __POS__ "syntactic symbols should never occur in processes"
      |	_ -> false) &&
...
let rec split_process bv bn = function
  ...
  | AnnBarrier _ ->
      Parsing_helper.internal_error __POS__ "Annotated barriers should not occur in split"
...
let rec check_no_barrier = function
  ...
  | AnnBarrier _ ->
      Parsing_helper.internal_error __POS__ "Annotated barriers should not occur in check_no_barrier"
...
let annotate_full compile_for_equiv barrier_tags p =
  ...
  let rec annotate = function
    ...
    | AnnBarrier _ ->
	Parsing_helper.internal_error __POS__ "Annotated barriers should not occur in annotate"
  in
...
let rec compile = function
  ...
  | Barrier _ ->
      Parsing_helper.internal_error __POS__ "Unannotated barriers should not occur in compile"
...
let rec add_out barl vl1 vl2 p =
  ...
  | _ -> Parsing_helper.internal_error __POS__ "barl, vl1, and vl2 should have the same length in Proswapper.add_out"
...
let rec add_out_permut_list (next_f : process -> unit) barll vll p =
  ...
  | _ -> Parsing_helper.internal_error __POS__ "barll and vll should have the same length in Proswapper.add_out_permut_list"
...
let rec get_elem tag tag' seen_barl barl seen_vl vl =
  ...
  | _ -> Parsing_helper.internal_error __POS__ "barl and vl should have the same length in Proswapper.get_elem"
...
let rec fixed_add_out_permut_list permut barll vll p =
  ...
  | _ -> Parsing_helper.internal_error __POS__ "barll and vll should have the same length in Proswapper.fixed_add_out_permut_list"
