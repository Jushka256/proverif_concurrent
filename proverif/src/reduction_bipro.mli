...
val do_reduction : ?id_thread:int -> ?bad_reached_in_lemmas:bool -> Pitypes.realquery option -> (Pitypes.realquery_e * bool) list -> Types.fact_tree -> bool

exception FailOnlyOnSide of int
val bi_action: (int -> 'a) -> 'a * 'a
val term_evaluation_fail : ?id_thread:int -> term -> int -> term
val term_evaluation_to_true : ?id_thread:int -> term -> int -> term
...
val match_pattern: ?id_thread:int -> pattern -> int -> term -> unit
