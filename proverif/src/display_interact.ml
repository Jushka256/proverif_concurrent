...
module LangGtk =
  struct
    ...
    let display_lemma_link lem =
      let type_lem = match lem.Types.l_origin_kind with
        | Types.Axiom -> "axiom "
        | Types.Lemma -> 
            if lem.l_induction <> None 
            then "inductive lemma "
            else "lemma "
        | Types.Restriction -> "restriction "
      in
      add_buffer (type_lem ^ (string_of_int lem.l_index) ^ " ")
    ...
    let leq_connective = esc_s "<="

    let le_connective = esc_s "<"

    let at_connective = esc_s "@"
