...
let rec get_ident_any state s ext =
  try
    ...
    | _ -> internal_error __POS__ "Only Var, Name, Fun should occur in the environment in the untyped front-end"
  with Not_found ->
...
let check_real_query_top state = function
	...
	| QSEvent(inj,ord_data,None,FunApp(f, sid::l)) ->
	    QSEvent(inj,ord_data, None,FunApp(f, non_compromised_session::l))
	| QSEvent _
        | QSEvent2 _->
  ...
  match term with
	...
	with Not_found ->
	  try
		...
		check_single (!glob_table) ext s;
		if fst r.f_type == Param.tmp_type then
		  internal_error __POS__ "Arity of a name uninitialized"
		else
		  FunApp (r, Terms.var_gen (fst r.f_type))
	    | _ -> internal_error __POS__ "should not find var/fun here"
	  with Not_found ->
