                          ...
                          Parsing_helper.debug (fun () ->
                            let check_one_term = 
                              Terms.iter_variables (fun v ->
                                if Terms.get_link v <> NoLink then assert(false)
                              )
                            in
        ...
        begin
          try
            TermsEq.unify_modulo (fun () ->
              ...
              let subst =
                List.fold_left (fun acc x -> match Terms.get_link x with
                  | NoLink -> acc
              ...
              List.iter (fun (x,_) -> Terms.link_unsafe x NoLink) subst;
              let hist' =
                Terms.auto_cleanup (fun () ->
                  let concl' = Terms.copy_fact2 cl.Ord.conclusion in
                  let hypl' = List.map(fun h -> Terms.copy_fact2 (HypOrd.get_fact h)) cl.Ord.hypotheses in
              ...
              List.iter (fun (x,t) -> Terms.link_unsafe x (TLink t)) subst;
