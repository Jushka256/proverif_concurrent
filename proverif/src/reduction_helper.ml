...
let get_process pi_state =
  ...
  | Equivalence _ -> Parsing_helper.internal_error __POS__ "[reduction_helper.get_process] Unexpected query."
  | SingleProcess(p,_) -> p
...
let rec find_min_choice_phase current_phase = function
  ...
  | Barrier _ | AnnBarrier _ ->
     Parsing_helper.internal_error __POS__ "Barriers should not appear here (6)"
...
let prove_att_phase_query n = function
  | PutBegin _ -> false
  | QSecret _ -> Parsing_helper.internal_error __POS__ "Reduction_helper.prove_att_phase: QSecret should have been encoded"
...
let prove_att_phase_tquery n = function
  ...
  | QueryToTranslate _ ->
      Parsing_helper.internal_error __POS__ "Reduction_helper.prove_att_phase: queries should have been translated"
  | _ -> false
...
||
  (List.exists (function
    | LemmaToTranslate _ -> Parsing_helper.internal_error __POS__ "Reduction_helper.prove_att_phase: lemma should have been translated"
    | Lemma lem ->
	...
	| KRestriction ->
	    List.exists (fun lem1 ->
	      ...
	      | PutBegin _ -> false
	      | QSecret _ -> Parsing_helper.internal_error __POS__ "Reduction_helper.prove_att_phase: lemma cannot be QSecret"
...
let rec split_events_list inj_events events std_pred user_def_pred constra subterm = function
  ...
  | (QSEvent(Some _,_,_,_) as inj_goal)::rest -> split_events_list (inj_goal::inj_events) events std_pred user_def_pred constra subterm rest
  | (QSEvent _ as goal)::rest -> split_events_list inj_events (goal::events) std_pred user_def_pred constra subterm rest
  | (QFact({p_info = AttackerBin _| Attacker _| Mess _ | MessBin _| Table _| TableBin _;_},_ ,_) as goal)::rest ->
      split_events_list inj_events events (goal::std_pred) user_def_pred constra subterm rest
  | (QFact(p,_,_) as goal)::rest when Terms.is_user_defined p ->
      split_events_list inj_events events std_pred (goal::user_def_pred) constra subterm rest
  | (QFact({p_info = Subterm _;_},_,_) as goal)::rest ->
  ...
  | _ -> Parsing_helper.internal_error __POS__ "[reduction_helper.split_events_list] Unexpected facts in the premise of a query."

let remove_injectivity = function
  | QSEvent(_,ord_data,occ,ev) ->
      assert(occ = None);
      QSEvent(None,ord_data,None,ev)
  | ev -> ev
...
let rec split_user_defined accu accu_p = function
  ...
  | (QFact(p,_,_) as goal) :: q when Terms.is_user_defined p -> split_user_defined accu (goal::accu_p) q
...
(** 
  ...
  Nested events corresponds to events that are added when proving a nested query. Thus there are none
  initially and only occurs during the verification of a query with nested queries.  *)
      ...
      then
        let evl' =
          match split_events_list [] [] [] [] [] [] evl with
            | [], [], [], [], [], [] -> Parsing_helper.internal_error __POS__ "[check_inj_coherent_r] Premises should contain at least one fact."
            ...
            | [], [QSEvent(None,ord_data,occ,ev)], std_pred,user_def_pred,constra,subterm ->
                ((QSEvent(Some (Param.fresh_injective_index ()), ord_data,occ,ev)) :: std_pred) @ user_def_pred @ constra @ subterm
...
let rec occurs_var_proc v = function
  ...
  | Barrier _ | AnnBarrier _ ->
     Parsing_helper.internal_error __POS__ "Barriers should not appear here (13)"
...
let get_need_vars pi_state s =
  let need_vars_in_names =
    ...
    | Function _ ->
       Parsing_helper.internal_error __POS__ "pi_need_vars_in_names must be computed before Reduction_helper.get_need_vars"
    | Computed x -> x
  in
...
let final_check (env, args, need_list) sf =
  ...
  | Some args_content ->
    List.iter (fun (b, n) ->
      if !n = 0 then
        Parsing_helper.internal_error __POS__ ("variable " ^ (Display.string_of_binder b) ^ " not found at restriction " ^ (Display.string_of_fsymb sf))) args_content
...
let rec extract_name_params_types sf include_info l lt =
  ...
  | _ -> Parsing_helper.internal_error __POS__ "lists should have same length in extract_name_params_types"
...
let rec add_at n a' l =
  ...
  match l with
    [] -> Parsing_helper.internal_error __POS__ "add_at"
  | (a::l) -> a::(add_at (n-1) a' l)

(* create a list with [n] elements [a] *)
let create n a =
  if n < 0
  then Parsing_helper.internal_error __POS__ "[reduction_helper.create] interger should be positive.";
...
let rec get_until_pos pos l =
  ...
  then []
    ...
    match l with
      | [] -> Parsing_helper.internal_error __POS__ "[reduction_helper.get_until_pos] Position outside of bound of the list"
...
let rec get_name_charac t = match t with
  | FunApp({f_cat = Name { prev_inputs_meaning = sl}} as f,l) ->
      let rec find_first_sid sl l = match (sl,l) with
        | [],[] -> [f]
...
let rec match_equiv_list next_f l1 l2 =
  ...
  | _ -> Parsing_helper.internal_error __POS__ "match_equiv_list"
...
let rec process_subst p n1 n2 =
  ...
  | AnnBarrier _ ->
     Parsing_helper.internal_error __POS__ "Annotated barriers should not appear here (14)"
...
let rec collect_constraints_rec accu tree = match tree.desc with
  ...
  | FRule(_,_,constra,sons,added_dl) ->
      accu := wedge_constraints constra !accu;
      List.iter (collect_constraints_rec accu) sons;

      List.iter (fun (_,a_constra,fact_l) ->
        accu := wedge_constraints a_constra !accu;
        List.iter (collect_constraints_rec accu) fact_l
      ) added_dl
      ...
      begin
        match Terms.get_link v with
          TLink t ->
            ...
            Terms.link_unsafe v (TLink2 t'); (* Store the image of the translated term, to avoid redoing work *)
            t'
        | TLink2 t' -> t'
...
let getphase p =
  ...
  | Table n | TableBin n ->
      n
  | _ -> Parsing_helper.internal_error __POS__ "Bad predicate for getphase"
...
let rec check_several_patterns_rec = function
  ...
  | FunApp(f,l) ->
	  ...
	  begin
	    ...
	    | _ -> Parsing_helper.internal_error __POS__ "Reduction_helper.check_several_patterns_rec: f_initial_cat Red expected"
	  end
...
let reduction_check_several_patterns occ =
  let terms_to_add_in_name_params =
    match (!Param.current_state).pi_terms_to_add_in_name_params with
      Unset -> Parsing_helper.internal_error __POS__ "pi_terms_to_add_in_name_params should be set"
    | Set x -> x
  in
  (!Param.eq_in_names) &&
...
let is_success_corresp_goal = function
    CorrespGoal (_,l,_) -> List.for_all is_success_corresp_goal1 l
  | _ -> false

(* [get_corresp_goals tree] returns the facts that the derivation
  concludes and the facts that have been added for the second derivation (liveness).
  ...
  Nested queries are constructed with a clause:
    Rule(-1,GenerationNested,hypl,concl,true) where hypl = old_main_fact :: nested_fact_l
    and [old_main_fact] is previous conclusion and [nested_fact_l] is the list of nested facts
    added in the conclusion.

  Injective queries are constructed with a clause 
    Rule(-1,GoalInjective,hyp,concl,constra) where hyp = [concl1;concl2] are the conclusion
    of the two merged clauses.
  *)
...
let retrieve_goals_from_id id_goals goal_list =
  ...
  let rec go_through i id_l g_l = match id_l, g_l with
    | [], _ -> []
    | _, [] -> Parsing_helper.internal_error __POS__ "[reduction_helper.retrieve_goals_from_id] Unexpected index"
...
let get_corresp_goals tree =
  ...
  (* We first retrieve the real goals + nested goals *)

  let rec explore_tree i tree = match tree.thefact with
    | Pred({p_info = Combined _},_) ->
        begin match tree.desc with
          | FRule(_,(GoalCombined | GoalInjective),_,sons,_) ->
              explore_tree_list i sons
          | FRule(_,GenerationNested,_,main_son::nested_sons,_) ->
          ...
          | _ -> 
            Parsing_helper.internal_error __POS__ "When a derivation concludes a combined predicate, it should have a clause as last step"
        end
  ...
  (* We complete by retrieving the facts added for liveness *)
  let nb_pure_goals = List.length goals in

  let acc_goals = ref [] in

  let rec explore_liveness tree = match tree.desc with
    | FRule(_,_,_,sons,added_dl) ->
        List.iter explore_liveness sons;
        List.iter explore_added_derivation added_dl;
    | FEquation tree' 
    | FRemovedWithProof tree' -> explore_liveness tree'
    | _ -> ()

  and explore_added_derivation (lbl,_,hypl) = match lbl with
    | LLiveness -> 
        List.iter (fun tree -> 
          if not (List.exists (Termslinks.equal_closed_facts tree.thefact) !acc_goals)
          then acc_goals := tree.thefact :: !acc_goals
        ) hypl
    | _ -> ()
  in

  explore_liveness tree;


  (nb_pure_goals,goals @ !acc_goals,id_real_goal)
...
let update_corresp_goal state_goal info_event test =
  ...
  let new_goal =
    match state_goal with
      | CorrespGoal(nb_initial_goals,l,ids) ->
          CorrespGoal(nb_initial_goals,
            List.map (function
...
let has_name_e = function
  ...
  | QSEvent2(_,t,t') -> has_name_t t || has_name_t t'
  | QGr _ -> Parsing_helper.internal_error __POS__ "[has_name_e] QGr should not occur in query during reduction."
  ...
  | QMax(t,args)
  | QMaxq(t,args) -> has_name_t t || List.exists has_name_t args
