...
let init_kinds d =
  ...
  List.iter (function
    ...
    | TEventDecl((e,_),_,_) -> Hashtbl.add kinds e "\\kwe"
...
| eof { internal_error __POS__ ((get_extent_string true (List.hd !comment_extent_list)) ^ "Unterminated comment") }
...
| eof
    { internal_error __POS__ ((get_extent_string true (extent lexbuf)) ^ "Unterminated string (should have been detected in previous pass)") }
