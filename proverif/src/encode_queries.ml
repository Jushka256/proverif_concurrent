...
let simplify_temporal_constraints at_constr =
  ...
  let assoc_new_var (t, ext) = match t with
    | Var v ->
        ...
        | NoLink -> 
            ...
            Terms.link_unsafe v (VLink v_nat);
            mapping := (v_nat,(v,ext)) :: !mapping;
            Var v_nat
        | VLink v_nat -> Var v_nat
        | _ -> Parsing_helper.internal_error __POS__ "Unexpected link."
        end
  ...
  List.iter (fun (_,(v,_)) -> Terms.link_unsafe v NoLink) !mapping;
