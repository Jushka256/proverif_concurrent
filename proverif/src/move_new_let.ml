...
let push is_new put_ins occurs needed_by_new occurs_new_args proc =
  ...
  let rec push_top proc = match proc with
    ...
    | AnnBarrier _ -> Parsing_helper.internal_error __POS__ "[push_let] Unexpected process."
...
let move_process need_vars_in_names lets_never_fail proc =
  ...
  let rec move = function
    ...
    | AnnBarrier _ ->  Parsing_helper.internal_error __POS__ "[move_let_process] Unexpected process."
  in
...
let move lets_never_fail pi_state =
  ...
  let need_vars_in_names =
    ...
    | Computed l -> l
    | _ -> Parsing_helper.internal_error __POS__ "[move_let] need_vars_in_names should have been computed."
  in
  ...
  try
    let process_query' =
      ...
      | Equivalence _ -> Parsing_helper.internal_error __POS__ "[move_let] Query should have been encoded."
