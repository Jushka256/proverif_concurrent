...
let async pool f =
  ...
  Multi_channel.send pd.task_chan (Work (fun i -> step (do_task (f i)) p));
  p
...
let setup_pool ?name ~num_domains () =
  ...
  let domains = Array.init num_domains (fun i ->
    Domain.spawn (fun _ -> worker (i+1) task_chan))
  in
...
let parallel_for_reduce ?(chunk_size=0) ~start ~finish ~body pool reduce_fun init =
  ...
  let rec work s e =
    if e - s < chunk_size then
      ...
      let d = s + ((e - s) / 2) in
      let p = async pool (fun _ _ -> work s d) in
...
let parallel_for ?(chunk_size=0) ~start ~finish ~body pool =
  ...
  let rec work pool fn s e =
    if e - s < chunk_size then
      ...
      let d = s + ((e - s) / 2) in
      let left = async pool (fun _ _ -> work pool fn s d) in
      work pool fn (d+1) e;
      await pool left
    end
  in
