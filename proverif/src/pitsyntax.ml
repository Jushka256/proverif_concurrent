...
let build_q_state must_encode_names pi_state =
  ...
  | _ ->
      Parsing_helper.internal_error __POS__ "glob_table should be set"
...
let rec check_event state (f,e) = match f with
  ...
  | PGFunApp(("event",e'),tl0,at_op) ->
      ...
      if !Param.key_compromise == 0 then
        ...
        QSEvent(None,ord_data,None,FunApp((get_event_fun state.q_env f (Param.sid_type :: tyl')),(Terms.new_var_def_term Param.sid_type)::tl'))
...
let rec check_ev_list state = function
  ...
  | ev ->
	...
	| QGr _ -> Parsing_helper.internal_error __POS__ "QGr should be used only with time variables"
        ...
        | QFact(p,_,_) ->
            ...
            ev'
        | QSEvent(Some _,_, _,_) when state.q_is_lemma <> None ->
...
let rec check_hyp state = function
  ...
  | ev ->
      ...
      if state.q_is_lemma <> None then
        begin
          match ev' with
          | QSEvent(Some _,_,_,_) -> Parsing_helper.input_error "Declared lemmas, axioms, and restrictions should not contain injective events." (snd ev)
          | _ -> ()
        end;
      QEvent(ev')
...
let check_variables_in_premise e evl =
  ...
  List.iter (function
    | QSEvent(_,_,_,ev) -> Terms.get_vars_acc other_facts_vars ev
    | QSEvent2(_,ev1,ev2) ->
        Terms.get_vars_acc other_facts_vars ev1;
        Terms.get_vars_acc other_facts_vars ev2
    | QFact(_,_,args) -> List.iter (Terms.get_vars_acc other_facts_vars) args
    | _ -> internal_error __POS__ "[check_variables_in_subterm_predicates] Expecting a list of predicate corresponding to the premise of a query."
  )  other_facts;
  ...
  | PPutBegin(i, l) ->
      if state.q_is_lemma <> None then
        begin
          ...
          | [] -> internal_error __POS__ "put_begin with empty list"
        end;
      let l' =
        List.map (fun (s,e) ->
                try
...
let put_user_defined_end_of_premise = function
  | Before(evl,qconcl) ->
      let rec explore accu accu_p = function
        ...
        | (QFact(p,_,_) as goal) :: q when Terms.is_user_defined p -> explore accu (goal::accu_p) q
...
let transl_option_lemma_query kind_lemma options =
  ...
  let concl_before_premise = ref !Param.conclusion_before_premise in
  ...
  let concl_before_premise_option = ref false in
  ...
  List.iter (fun (str,ext) -> match str with
    ...
    | "maxSubset" ->
        if kind_lemma = Some KAxiom || kind_lemma = Some KRestriction
        then input_error ((print_kind_lemma kind_lemma) ^" cannot have the option maxSubset. Only lemmas and queries can.") ext;
    ...
    | "proveAll" ->
        if kind_lemma = Some KAxiom || kind_lemma = Some KRestriction
        then input_error ((print_kind_lemma kind_lemma) ^" cannot have the option proveAll. Only lemmas and queries can.") ext;
        if !max_subset_option
    ...
    | "noneVerif" ->
        ...
        verif_option := true
    | "discardSat" ->
        input_warning "Deprecated! discardSat does not have any effect anymore." ext
    | "discardVerif" ->
        input_warning "Deprecated! discardVerif does not have any effect anymore." ext
    | "fullSat" ->
        if !sat_option
        then input_error "An option for the application of this lemma during the saturation procedure as already been declared." ext;
        sat_app := true;
    ...
    | "fullVerif" ->
        ...
        verif_option := true
    | "instantiateSat" ->
        input_warning "Deprecated! instantiateSat does not have any effect anymore." ext
    | "instantiateVerif" ->
        input_warning "Deprecated! instantiateVerif does not have any effect anymore." ext
    | "induction" ->
        if !induction_option
    ...
    | "keep" ->
        ...
        keep := true
    | "conclBeforePremise" ->
        if !concl_before_premise_option
        then input_error "The option noOrderOnConcl or noOrderOnConcl has already been set." ext;
        concl_before_premise := true;
        concl_before_premise_option := true
    | "noOrderOnConcl" ->
        if !concl_before_premise_option
        then input_error "The option noOrderOnConcl or noOrderOnConcl has already been set." ext;
        concl_before_premise := false;
        concl_before_premise_option := true
  ...
  let solve_status = 
    { 
      ...
      verif_app = !verif_app;
      induction = !induction;
      concl_before_premise = !concl_before_premise
    }
  in
...
let transl_query (env,q,options) pi_state =
  ...
  let query_list = List.map (fun (q, ext) -> Reduction_helper.check_inj_coherent q, ext) q' in
  
  CorrespQuery(query_list, solve_status)
...
let transl_lemma kind_lemma (env,q,options) pi_state =
  ...
  List.iter2 (fun q1 q1' ->
        ...
        if is_equivalence_query then
          ...
          begin
            let queryl =
              ...
              | Equivalence _ -> internal_error __POS__ "Should be handled before"
              | SingleProcessSingleQuery(_,query) -> [query]
              | SingleProcess(_,queryl) -> queryl
            in
            if not (List.exists (function
              | QueryToTranslate _ -> internal_error __POS__ "transl_lemma: Queries should have been translated"
              | CorrespQEnc _ | ChoiceQEnc _ -> internal_error __POS__ "transl_lemma: Queries should not have been encoded"
    ...
    | _, None -> ()
    | _ -> internal_error __POS__ "[transl_lemma] Lemmas should be encoded as RealQuery."
    ) q q';

  let (solve_status,keep,remove_events) = transl_option_lemma_query (Some kind_lemma) options in

  let lemma_list =
    List.filter_map (function
      | Some((RealQuery _ as rq ,ext), ror_opt) ->
          Some {
            ql_query = (Reduction_helper.check_inj_coherent rq,ext);
	      (* The goal of check_inj_coherent here is to order the premises canonically *)
...
let check_gfact_format state concl_opt ((s, ext), tl, n) =
  ...
  | "event" ->
      ...
      if not !is_concl
      then input_error "Nounif declaration on an event must have the option 'conclusion'" ext;
    ...
    begin
      ...
      List.iter (fun (i,arg) -> match i with
        ...
        | "ignoreAFewTimes",ext ->
            ...
            induc_verif_op := true;
            hyp_op := true
        | _, ext -> input_error "The only options available for nounif declaration are: hypothesis, conclusion and ignoreAFewTimes." ext
      ) opt;

      let op_acc = ref [] in
      if !hyp_op then op_acc := Hypothesis :: !op_acc;
      if !concl_op then op_acc := Conclusion :: !op_acc;
      if !induc_verif_op then op_acc := Ignore :: !op_acc;
...
let memberOptim = ref ([]: (predicate * funsymb option) list)
...
let check_equations l eqinfo =
  ...
  in
  let l' = List.filter_map (fun (env, t) ->
    ...
    | (_,t1)::_ ->
      begin
          ...
          List.map (fun (var_def,t) ->
            ...
            match t' with
              | PFunApp(("=",_), [(PFunApp((f',ext'),l1),_);t2]),_ ->
                  ...
                  Terms.get_vars_acc var_list_rhs rhs;
        ...
        match ty_red_list with
          | [] -> internal_error __POS__ "reduction with empty list"
          | (tylhs,tyrhs)::r ->
              ...
              let red_list' =
                begin
                  ...
                  Terms.cleanup ();
                  try
                    let side_cond' = TermsEq.simplify_constraints_simple None side_cond in
...
let predicates_to_solve_twice = ref []
...
let check_pred (c,ext) tl info =
  ...
  let r = { p_name = c; p_type = tyl; p_prop = prop;
            ...
            p_record = 0 }
  in
  let solveOnce = List.exists (function "solveOnce", _ -> true | _ -> false) info in
  let solveTwice = 
    List.exists (function 
      | "solveTwice", ext -> 
          if solveOnce then input_error "A predicate cannot be declared with both options solveOnce and solveTwice." ext;
          true
      | _ -> false
    ) info 
  in 
  if solveOnce then predicates_to_solve_twice := (r,false) :: !predicates_to_solve_twice;
  if solveTwice then predicates_to_solve_twice := (r,true) :: !predicates_to_solve_twice;

  if prop land Param.pred_ELEM != 0
  then memberOptim := (r,None) :: !memberOptim;
...
let rec check_hyp is_simple (hyp_accu,constra_accu) env (fact, ext) =
  ...
  | PFunApp((f,fext) as p, l) ->
      ...
      | ("<>" | "=" | "&&"), _ -> internal_error __POS__ ("Bad arity for special function " ^ f)
      | _ ->
...
let check_clause = function
  | (env, PFact(c)) ->
     ...
     let (_,e) = c in
     Transl_helper.check_membership e memberOptim [] Terms.true_constraints concl;
  ...
  | (env, PClause(i,c)) ->
     ...
     let concl = check_simple_fact env c in
     let (_,e) = c in
     Transl_helper.check_membership e memberOptim hyp constra concl;
  ...
  | (env, PEquiv(i,c,select)) ->
      ...
      if not (Terms.is_true_constraints constra) then
        Parsing_helper.internal_error __POS__ "Inequality constraints not allowed in equivalences, should be eliminated by check_hyp true\n";
      let concl = check_simple_fact env c in
      let (_,e) = c in
      Transl_helper.forbid_equiv_membership e memberOptim hyp concl;
...
let events_to_solve_twice = ref []

let check_event (name, ext) argtypes options =
  let tyarg = List.map get_type argtypes in
  ...
  begin match options with
    | [("solveOnce",_),_] -> events_to_solve_twice := (r,false) :: !events_to_solve_twice
    | [("solveTwice",_),_] -> events_to_solve_twice := (r,true) :: !events_to_solve_twice
    | [] -> ()
    | _ -> input_error "Events can only have either solveOnce or solveTwice as options." ext
  end;
...
let rec et_term_never_fail t =
  ...
  | ET_FunApp(f,args) ->
      begin match f.f_initial_cat with
        ...
        | SpecVar _ | Syntactic _ | General_var | General_mayfail_var -> Parsing_helper.internal_error __POS__ "[et_term_never_fail] Unexpected symbol in processes."
      end
      ...
      let rec get_lets_args = function
        ...
        | b::l ->
            ...
            | ETLink t ->
                ...
                in
                ((b', t')::lets, b'::l')
            | _ -> Parsing_helper.internal_error __POS__ "unexpected link in Pitsyntax.check_term"
      in
      let lets, l' = get_lets_args l in
      (lets, Some l')
        ...
        match get_apply_symb env (s,ext) type_list with
          (EFun f, result_type) ->
            if (f.f_options land Param.fun_TYPECONVERTER != 0) && (Param.get_ignore_types()) then
              ...
              | _ -> internal_error __POS__ "Type converter functions should always be unary"
            else
...
and check_ep_list ext environment type_pat_list_opt pat_list env = match pat_list,type_pat_list_opt with
  ...
  | pat::pat_l, ty_pat::ty_pat_l ->
      ...
      (pat::pat_l, env'')
  |_,_ -> internal_error __POS__ "[check_ep_list] Size of pattern list and type list should be the same"
...
and expand_event_insert_arg t =
  ...
  | ET_FunApp(f,l) ->
      ...
      proc_layer'
  | _ ->
      internal_error __POS__ "argument of event or insert should be a function application"
...
let rec rename_format = function
  ...
  | PFName _ -> internal_error __POS__ "Names not allowed in formats with -in pi"
...
let rename_decl = function
  ...
  | TEventDecl(i,l,opt) -> TEventDecl(rename_ie i, List.map rename_ie l,opt)
...
let rec check_one = function
  ...
  | TEventDecl(i, args,opt) -> check_event i args opt
  ...
  | TDefine _ | TExpand _ ->
      internal_error __POS__ "macros should have been expanded"

  | TSet _ -> internal_error __POS__ "set declaration should have been handled before"
...
let interpret_setting (p,ext) v =
  ...
  | "conclBeforePremise", _ -> Param.boolean_param Param.conclusion_before_premise p ext v
  | "orderedSaturation", _ -> Param.boolean_param Param.ordered_saturation p ext v
  ...
  | "noselectIgnoreConclusion", S (("false" | "no"),_) -> Param.nounif_ignore_conclusion:= false
  | "noselectIgnoreConclusion", S (("true" | "yes"),_) -> Param.nounif_ignore_conclusion:= true
  | "removeEventsForLemma", _ -> Param.boolean_param Param.event_lemma_simplification p ext v
  | "saturationApplication", S (("false" | "no" | "none"),_) -> Param.sat_application:= false
  | "saturationApplication", S (("true" | "yes" | "full"),_) -> Param.sat_application:= true
  | "saturationApplication", S ("discard",_) -> 
      Parsing_helper.input_warning "Deprecated! The argument `discard' is now ignored." ext
  | "saturationApplication", S ("instantiate",_) -> 
      Parsing_helper.input_warning "Deprecated! The argument `instantiate' is now ignored." ext
  | "verificationApplication", S (("false" | "no" | "none"),_) -> Param.verif_application:= false
  | "verificationApplication", S (("true" | "yes" | "full"),_) -> Param.verif_application:= true
  | "verificationApplication", S ("discard",_) -> 
      Parsing_helper.input_warning "Deprecated! The argument `discard' is now ignored." ext
  | "verificationApplication", S ("instantiate",_) -> 
      Parsing_helper.input_warning "Deprecated! The argument `instantiate' is now ignored." ext
...
let parse_file s =
  ...
  let pi_state =
    { pi_process_query = process_query;
      ...
      pi_events_to_solve_twice = !events_to_solve_twice;
      pi_predicates_to_solve_twice = !predicates_to_solve_twice;
      pi_requires_liveness = false
    }
  in
  reset();
  pi_state
