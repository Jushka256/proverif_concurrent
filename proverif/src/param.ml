...
let get_ignore_types () =
  match !ignore_types with
    NotSet -> Parsing_helper.internal_error __POS__ "ignore_types read before set"
  | Ignore -> true
  | NotIgnore -> false
...
type t_verbose_base = 
  | VBNone
  | VBAll
  | VBInterval of int * int

let verbose_base = ref VBNone
...
let string_of_pval = function
  | S (s,_) -> s 
  | I n -> string_of_int n 
  | Inter(n1,n2) -> (string_of_int n1) ^"-"^(string_of_int n2)
...
let boolean_param flag p ext v =
  ...
  | _ -> 
      Parsing_helper.input_error ("Unexpected value for parameter " ^ p ^ "=" ^ (string_of_pval v)) ext

let common_parameters p ext v =
  match (p,v) with
  | "color", _ -> boolean_param ansi_color p ext v
  | "verboseBase", S (("true"|"yes"),_) -> verbose_base := VBAll
  | "verboseBase", S (("false"|"no"),_) -> verbose_base := VBNone
  | "verboseBase", Inter(s1,s2) -> verbose_base := VBInterval(s1,s2)
...
let build_pred = function
  ...
  | Combined(nb_std,nb_user,pl) ->
      ...
      { p_name = "combined_" ^ (String.concat "_" (List.map (fun p -> p.p_name) pl));
        ...
        p_prop = 0;
        p_info = Combined(nb_std,nb_user,pl);
        p_record = 0 }
  | Subterm(t,t') ->
      { p_name = "subterm";
        p_type = [t;t'];
  ...
  | Block p ->
      ...
      then Parsing_helper.internal_error __POS__ "[param.build_pred] A predicate cannot be the block version of another blocking predicate.";
  ...
  | UserPred(Monom) | OtherInternalPred -> Parsing_helper.internal_error __POS__ "[param.build_pred] Unexpected info."
...
let inj_event_pred = { p_name = "occ-event"; p_type = [event_type;occurrence_type] ; p_prop = 0; p_info = OtherInternalPred; p_record = 0 }
...
let sat_application = ref true
let verif_application = ref true

let conclusion_before_premise = ref false
...
let nounif_ignore_conclusion = ref true
...
let dummy_pi_state =
  { pi_process_query = SingleProcess({ proc = Nil; bi_pro = false; display_num = None; construction = Initial_process}, []);
    ...
    pi_events_to_solve_twice = [];
    pi_predicates_to_solve_twice = []
...
let get_process_query pi_state =
  ...
  | _ -> Parsing_helper.internal_error __POS__ "SingleProcessSingleQuery expected in get_process_query"
...
let reset_param() =
  ...
  sat_application := true;
  verif_application := true;
  conclusion_before_premise := false;
