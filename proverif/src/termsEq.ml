...
let rec close_term_eq id_thread restwork = function
    Var x ->
      begin
        match Terms.get_link ~id_thread ~name:"close_term_eq" x with
...
module Make(H:HypSig)(C:ClauseSig with type hyp = H.hyp) : EqSig with type clause = C.clause =
struct
        ...
        close_constra_eq (fun constra ->
          let cl' =
            Terms.auto_cleanup (fun () ->
              let hyp_l' = List.map H.copy2 hyp_l in
              let concl' = copy_fact2 concl in
              let constra' = copy_constra2 constra in
  
              let histref = ref cl.history in
  
              (* Generate history w.r.t. hypotheses *)
              let rank = ref 0 in
              List.iter2 (fun hyp1 hyp1' ->
                if not (H.equal_facts hyp1 hyp1')
                then histref := HEquation(!rank, copy_fact2 (H.get_fact hyp1), copy_fact2 (H.get_fact hyp1'), !histref);
                incr rank
              ) cl.hypotheses hyp_l;
  
              let hist' = 
                if equal_facts cl.conclusion concl
                then !histref
                else HEquation(-1, concl', copy_fact2 cl.conclusion, !histref)
              in 
              { 
                hypotheses = hyp_l' ; 
                conclusion = concl';
          ...
          in
          Terms.auto_cleanup (fun () -> restwork cl')
        ) cl.constraints
      ) cl.conclusion
    ) cl.hypotheses
    ...
    close_hyp_list_modulo_eq (fun hyp_l ->
      close_constra_eq (fun constra ->
        let cl' =
          Terms.auto_cleanup (fun () ->
            let hyp_l' = List.map H.copy2 hyp_l in
            let concl' = copy_fact2 cl.conclusion in
            let constra' = copy_constra2 constra in
  ...
  let close_hyp_modulo_eq restwork cl =
    if hasEquations() then close_hyp_modulo_eq restwork cl else restwork cl
end
...
let rec close_term_destr_eq accu_constra restwork = function
    Var x ->
      begin
        match Terms.get_link ~name:"close_term_destr_eq" x with
          TLink t ->
          (* TO DO should I always recursively close links modulo equations? *)
...
let add_rewrite ((leq, req) as r) =
  ...
  Terms.get_vars_acc var_right req;
...
| _ -> Parsing_helper.internal_error __POS__ "Different length in greater_lpo_lex"
...
let record_eqs_internal equations_list =
  ...
  begin
	  ...
	  with
		...
		begin
		  ...
		  begin
			...
			print_string ".)\n"
		    | _ -> Parsing_helper.internal_error __POS__ "TermsEq: added to avoid warning for non-exhaustive pattern-matching"
		  end;
    ...
    List.iter (function
	  ...
	  Terms.get_vars_acc var_list_rhs req;
  ...
  with Not_found ->
    let f' = { f_name = (* "sy_" ^ *)
                 begin
                   match f.f_name with
                   | Fixed _ -> f.f_name
                   | Renamable _ -> Parsing_helper.internal_error __POS__ "get_syntactic only for fixed functions"
                 end;
               f_type = f.f_type;
               f_cat = Syntactic f;
...
let rec copy_remove_syntactic = function
  ...
  | Var v ->
      ...
      | VLink r -> Var r
      | _ -> internal_error __POS__ "unexpected link in copy_remove_syntactic"
...
let copy_remove_syntactic_ordering_data ord_data = 
  { ord_data with temp_var = copy_remove_syntactic_e_option ord_data.temp_var }

let copy_remove_syntactic_event = function
  | QSEvent(inj,ord_data,occ,t) -> QSEvent(inj,copy_remove_syntactic_ordering_data ord_data,copy_remove_syntactic_option occ,copy_remove_syntactic t)
  | QSEvent2(ord_data,t1,t2) -> QSEvent2(copy_remove_syntactic_ordering_data ord_data,copy_remove_syntactic t1, copy_remove_syntactic t2)
  | QFact(p,ord_data,args) -> QFact(p,copy_remove_syntactic_ordering_data ord_data,List.map copy_remove_syntactic args)
  ...
  | QMax(t,args) -> QMax(copy_remove_syntactic t,List.map copy_remove_syntactic args)
  | QMaxq(t,args) -> QMaxq(copy_remove_syntactic t,List.map copy_remove_syntactic args)
...
let rec remove_syntactic_term = function
 ...
 | Var v -> match Terms.get_link ~name:"remove_syntactic_term" v with
      NoLink -> Var v
...
let rec collect_unset_vars accu = function
  ...
  | Var v ->
      match Terms.get_link ~name:"collect_unset_vars" v with
	NoLink ->
	  if not (List.memq v (!accu)) then
	    accu := v :: (!accu)
      | TLink t -> collect_unset_vars accu t
      | _ -> internal_error __POS__ "unexpected link in collect_unset_vars"
...
and close_term_eq_synt_tail ?(id_thread=0) restwork f_clean f_Unify f_NoBacktrack = function
  | (Var x) as t ->
    begin
      match Terms.get_link ~id_thread ~name:"close_term_eq_synt" x with
...
and unify_modulo_tail ?(id_thread=0) restwork f_clean f_Unify f_NoBacktrack t1 t2 =
  close_term_eq_synt_tail ~id_thread (fun f_clean_1 f_Unify_1 f_NoBacktrack_1 t1 ->
    close_term_eq_synt_tail ~id_thread (fun f_clean_2 f_Unify_2 f_NoBacktrack_2 t2 ->
      ...
      | (Var v, _) ->
          begin
            match Terms.get_link ~id_thread ~name:"unify_modulo_tail" v with
            | NoLink ->
                begin
                  match t2 with
                  | Var v' ->
...
and unify_modulo_list_internal_tail ?(id_thread=0) restwork f_clean f_Unify f_NoBacktrack l1 l2 =
  ...
  | (a1::l1'), (a2::l2') ->
        ...
        unify_modulo_tail ~id_thread (fun f_clean_1 f_Unify_1 f_NoBacktrack_1 ->
          if not (List.exists (fun v -> Terms.get_link ~id_thread ~name:"unify_modulo_list_internal_tail" v != NoLink) (!unset_vars))  then
...
and unify_modulo_list_tail ?(id_thread=0) restwork f_clean f_Unify f_NoBacktrack l1 l2 =
  let rec add_unif_term restwork f_clean f_Unify f_NoBacktrack unif_to_do_left unif_to_do_right t1 t2 =
    ...
    | (Var v, _) ->
          begin
            match Terms.get_link ~id_thread ~name:"unify_modulo_list_tail" v with
            | NoLink ->
                begin
                  match t2 with
                  | Var v' ->
            ...
            | _ -> internal_error __POS__ "Unexpected link in unify_modulo_list (optimized) 1"
          end
      | (FunApp(f,_), Var v) ->
          begin
            match Terms.get_link ~id_thread ~name:"unify_modulo_tail" v with
            ...
            | TLink t2' -> add_unif_term restwork f_clean f_Unify f_NoBacktrack unif_to_do_left unif_to_do_right t1 t2'
            | _ -> internal_error __POS__ "Unexpected link in unify_modulo_list (optimized) 2"
          end
  ...
  and add_unif_term_list restwork f_clean f_Unify f_NoBacktrack unif_to_do_left unif_to_do_right l1 l2 = match l1,l2 with
    ...
    | [], _ | _, [] -> Parsing_helper.internal_error __POS__ "List should be of same size"
    | t1::q1, t2::q2 ->
...
let rec get_list l1 l2 =
  ...
  | _ -> internal_error __POS__ "Lists should have same length in get_list"
  ...
  begin
    ...
    then
      ...
      then raise TrueConstraint
        ...
        begin
          match Terms.get_link ~id_thread ~name:"elim_var_if_possible" v with
          | NoLink ->
          ...
          | TLink _ -> ()
          | _ -> Parsing_helper.internal_error __POS__ "unexpected link in elim_var_if_possible"
        end
  end
...
let rec get_status_natural_number ?(id_thread=0) nat_vars = function
  | Var v ->
      begin
        match Terms.get_link ~id_thread ~name:"get_status_natural_number" v with
...
let rec can_be_natural_number ?(id_thread=0) = function
  | Var v -> 
      begin
        match Terms.get_link ~id_thread ~name:"can_be_natural_number" v with
      ...
      try
        close_term_eq_synt id_thread (fun t3 -> match get_status_natural_number ~id_thread nat_vars t3 with
          | IsNat ->
              ...
              if List.for_all (fun v -> match Terms.get_link ~id_thread ~name:"simplify_is_not_nat" v with NoLink -> true | _ -> false) !accu_vars
              then raise FalseConstraint;
              can_be_nat := true;
              raise Unify
          | CanBeNat _ ->
   ...
   List.iter (fun (k1,k2,w) ->
     match distance.(i).(k1), distance.(i).(k2) with
       | N n1, N n2 ->
         if n1 + w < n2
         then raise FalseConstraint
       | N _, Infinity -> Parsing_helper.internal_error __POS__ "[TermsEq.algo_BellmanFord] Unexpected case"
       | _, _ -> ()
   ) edges
 done;
...
let update_keepvars ?(id_thread=0) keep_vars =
  ...
  List.iter (fun v -> match Terms.get_link ~id_thread ~name:"update_keepvars" v with
...
let rec make_disequation_from_unify ?(id_thread=0) keep_vars assoc_gen_with_var = function
  ...
  | (var::l) ->
      ...
      match Terms.get_link ~id_thread ~name:"make_disequation_from_unify" var with
        | NoLink -> l'
...
let unify_disequation ?(id_thread=0) nextf accu constra =
  ...
  List.iter (fun (t1,t2) -> get_vars_acc keep_vars t1; get_vars_acc keep_vars t2) constra;
...
let elim_universal_variable constra =
  ...
  let rec look_for_general_var constra_done = function
    ...
    | (Var v,_)::q when v.unfailing -> Parsing_helper.internal_error __POS__ "May-fail variables forbidden in inequalities (1)"
    | (_, Var v)::q when v.unfailing -> Parsing_helper.internal_error __POS__ "May-fail variables forbidden in inequalities (2)"
...
(*** Main simplification function
  Simple VS Optimal
  KeepSome VS PreservesAll via the argument keepvars_op

  simplify_constraints_simple
  simplify_constraints_optimal
***)
...
let simplify_constraints_keepvars ?(id_thread=0) f_next f_next_inst keep_vars c =
  ...
  get_equalities ~id_thread (fun eq_left eq_right assoc number_vertices distance ->
    ...
    Terms.auto_cleanup ~id_thread (fun () ->
      ...
      let is_nat2 =
        let geq_vars = ref [] in
        List.iter (fun (t1,_,t2) -> get_vars_acc geq_vars t1; get_vars_acc geq_vars t2) geq2;
...
let simplify_constraints_simple keepvars_op c =
  if Terms.is_true_constraints c
  then c
  else
    let c = Terms.copy_constra4 c in
    let vars = match keepvars_op with
      | None -> Terms.get_vars_constra c
      | Some get_vars -> get_vars ()
    in
    simplify_constraints_keepvars
      ...
      (fun c1 ->
        ...
        let c1' =
          List.fold_left (fun acc v -> match Terms.get_link ~name:"simplify_constraints_simple" v with
            | TLink t ->
      ...
      vars c
  [@ppwarning "For efficiency, check when we can remove the copy_constra4"]

let simplify_constraints_continuation keepvars_op f_next f_next_inst c =
  if Terms.is_true_constraints c
  then f_next c
  else 
    let vars = match keepvars_op with
      ...
      | Some get_vars -> get_vars ()
    in
    simplify_constraints_keepvars f_next f_next_inst vars c

let simplify_constraints_optimal keepvars_op f_next f_next_inst c = 
  if Terms.is_true_constraints c
  then f_next c
  else
    let vars = match keepvars_op with
    ...
    try
      if c.geq = []
      then simplify_constraints_keepvars f_next f_next_inst vars c
      else simplify_after_inst f_next f_next_inst [] vars c
    with FalseConstraint -> ()
...
let rec nat_of_closed_term = function
  | Var v -> begin 
      match Terms.get_link ~name:"nat_of_closed_term" v with
...
let implies_constraints ?(id_thread=0) nat_vars1 geq1_data constraints1 constraints2 =
  ...
  List.iter (fun (t1,_,t2) -> get_vars_acc nat_vars2 t1; get_vars_acc nat_vars2 t2) constraints2.geq;
  ...
  try 
    ...
    then
      begin 
        ...
        List.iter (get_vars_acc nat_vars1) constraints1.is_nat;
        List.iter (fun (t1,_,t2) -> get_vars_acc nat_vars1 t1; get_vars_acc nat_vars1 t2) constraints1.geq;
  ...
  check_after_inst (fun constra1 ->
    ...
    algo_BellmanFord (fun assoc number_vertices distance ->
      Terms.auto_cleanup (fun () ->
        ...
        List.iter (fun v -> match Terms.get_link ~name:"get_solution" v with
...
let implies_rew (leq1, req1, side_c1) (leq2, req2, side_c2) =
  ...
  try
    ...
    let vars = lazy
	(let vars = ref [] in
	List.iter (Terms.get_vars_acc vars) leq2;
	Terms.get_vars_acc vars req2;
	!vars)
    in
    let res = implies_constraints3 (Some (fun () -> Lazy.force vars)) side_c2 side_c1 in
    Terms.cleanup();
    res
  with NoMatch ->
    Terms.cleanup();
...
let close_destr_eq f l =
      ...
      | (req'::leq') ->
          close_constra_eq (fun side_c0 ->
            ...
            begin
              try
                let vars = ref [] in
                List.iter (Terms.get_vars_acc vars) leq1;
                Terms.get_vars_acc vars req1;
  ...
  List.iter (fun (leq, req, side_c) ->
    let var_list_rhs = ref [] in
    Terms.get_vars_acc var_list_rhs req;
    Terms.get_vars_acc_constra var_list_rhs side_c;
